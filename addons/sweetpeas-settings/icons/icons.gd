...
static func note_input_event(event: InputEvent) -> void:
	SweetInputDeviceTracker.note_event(event)

static func path_for_event(event: InputEvent) -> String:
	if event is InputEventJoypadButton:
		var e := event as InputEventJoypadButton
		return SweetControllerIconMap.path_for_button(
			e.button_index, SweetInputDeviceTracker.family_for_device(e.device)
		)
	if event is InputEventJoypadMotion:
		var e := event as InputEventJoypadMotion
		return SweetControllerIconMap.path_for_axis(
			e.axis, e.axis_value, SweetInputDeviceTracker.family_for_device(e.device)
		)
	if event is InputEventKey or event is InputEventMouseButton:
		return SweetKeyboardIconMap.path_for_event(event)
	return ""
...
static func texture_for_controller_device(family: String = "") -> Texture2D:
	var texture := SweetControllerIconMap.texture_for_device(family)
	if texture != null:
		return texture
