# settings_schema.gd
# A parsed and normalized instance of the config schema file.
# Construct, then call parse(); SettingSources.apply() fills in runtime values.
extends RefCounted
...
# LOADING
#================================================================================#
# Reads and normalizes the schema file into this instance. On failure the
# instance stays empty, so callers always get a usable object.
func parse() -> void:
	source_path = resolve_path()

	if not FileAccess.file_exists(source_path):
		push_error("Sweet Settings: no schema found at '%s'." % source_path)
		return

	var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(source_path))
	if not parsed is Dictionary:
		push_error("Sweet Settings: invalid schema at '%s'." % source_path)
		return

	_build(parsed)

static func resolve_path() -> String:
	var default_path: String = AddonPaths.CONFIG_DIR.path_join(DEFAULT_SCHEMA_FILE)
	...
	for file_name in DirAccess.get_files_at(AddonPaths.CONFIG_DIR):
		...
		if clean_name == DEFAULT_SCHEMA_FILE or clean_name.get_extension().to_lower() != "json":
			continue
		custom_paths.append(AddonPaths.CONFIG_DIR.path_join(clean_name))
	...
	if custom_paths.size() > 1:
		push_warning(
			"Sweet Settings: found %d custom schemas in '%s'; using '%s'."
			% [custom_paths.size(), AddonPaths.CONFIG_DIR, custom_paths[0]]
		)
...
func _normalize_section(raw_section: Dictionary) -> Dictionary:
	...
	if id.is_empty():
		push_error("Sweet Settings: a section is missing its 'id'; skipping it.")
		return {}
...
func _normalize_section_component(
	...
	if raw is String or raw is StringName:
		...
		push_error(
			"Sweet Settings: section '%s' %s entry must be a string or object."
			% [section_id, slot]
		)
		return {}
	...
	if component["type"].is_empty():
		push_error(
			"Sweet Settings: section '%s' %s entry is missing 'type'."
			% [section_id, slot]
		)
		return {}
	...
	if raw_entry.has(KEYS_FIELD):
		...
		for item in raw_entry.get(KEYS_FIELD, []):
			...
			if raw_setting.is_empty():
				push_error(
					"Sweet Settings: a '%s' item in section '%s' must be a string or object."
...
func _register_setting(
	...
	if _by_key.has(setting["key"]):
		push_error(
			"Sweet Settings: duplicate setting key '%s'; keeping the first one."
			% setting["key"]
		)
		return []
...
func _normalize_setting(raw_setting: Dictionary, shared: Dictionary, section_id: String) -> Dictionary:
	...
	var key := str(setting.get("key", "")).strip_edges()
	if key.is_empty():
		push_error("Sweet Settings: a setting in section '%s' has no 'key'." % section_id)
		return {}
	...
	if setting["type"].is_empty():
		push_error("Sweet Settings: setting '%s' has no 'type'; skipping it." % key)
		return {}
	...
	if not setting.has("default"):
		if setting["type"] == "keybind":
			...
			if options.is_empty():
				push_error("Sweet Settings: setting '%s' has no 'default'; skipping it." % key)
				return {}
