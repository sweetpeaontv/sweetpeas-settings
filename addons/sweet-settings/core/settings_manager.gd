...
const SettingApplier = preload("uid://bb2oa0wnxndx1")
const SettingSources = preload("uid://biwn3sfivkq6w")
const SettingsData = preload("uid://defj5f8oxiba6")
const SettingsI18n = preload("uid://csepy51qxiem1")
const SettingsSchema = preload("uid://dyur3ipiqsfn5")
...
func _ready() -> void:
	...
	# parse() reads the JSON; apply() fills options/defaults from the runtime
	# environment (displays, locales, Input Map). Composed here so the parser
	# stays independent of the injectors.
	schema = SettingsSchema.new()
	schema.parse()
	SettingSources.apply(schema)

	data = SettingsData.new(schema)
	data.load_saved()
...
func _unknown_key_message(key: String) -> String:
	...
	if suggestion.is_empty():
		return "Sweet Settings: no setting named '%s' in '%s'." % [key, schema.source_path]

	return (
		"Sweet Settings: no setting named '%s' in '%s'. Did you mean '%s'?"
