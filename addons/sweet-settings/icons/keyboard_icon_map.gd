...
static func path_for_key(key: Key) -> String:
	...
	return IconPaths.join(_SUBDIR, file) if not file.is_empty() else ""
...
static func texture_for_key(key: Key) -> Texture2D:
	return IconPaths.load_texture(path_for_key(key))

static func texture_for_mouse_button(button: MouseButton) -> Texture2D:
	return IconPaths.load_texture(path_for_mouse_button(button))
